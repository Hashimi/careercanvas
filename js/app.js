...
function addExperience() { cvData.experiences.push({ role: "", company: "", date: "", desc: "" }); renderEditorInputs(); renderPreview(); }
function addEducation() { cvData.educations.push({ degree: "", school: "", date: "" }); renderEditorInputs(); renderPreview(); }
function addLanguage() { cvData.languages.push({ lang: "", level: "" }); renderEditorInputs(); renderPreview(); }
function addSkill() { if (!cvData.skills) cvData.skills = []; cvData.skills.push({ name: "", level: 50 }); renderEditorInputs(); renderPreview(); }
...
function renderEditorInputs() {
    ...
    if (cvData.skills) {
        skillsContainer.innerHTML = cvData.skills.map((skill, index) => `
            <div class="flex gap-2 items-center">
                ...
                <button onclick="removeItem('skills', ${index})" class="text-red-400"><i class="fas fa-times"></i></button>
            </div>
        `).join('');
    }
    ...
    expContainer.innerHTML = cvData.experiences.map((exp, index) => `
        <div class="bg-gray-50 p-3 rounded-lg border relative">
            <button onclick="removeItem('experiences', ${index})" class="absolute top-2 end-2 text-red-400"><i class="fas fa-trash"></i></button>
    ...
    eduContainer.innerHTML = cvData.educations.map((edu, index) => `
        <div class="bg-gray-50 p-3 rounded-lg border relative">
            <button onclick="removeItem('educations', ${index})" class="absolute top-2 end-2 text-red-400"><i class="fas fa-trash"></i></button>
    ...
    langContainer.innerHTML = cvData.languages.map((lang, index) => `
        <div class="flex gap-2">
            ...
            <button onclick="removeItem('languages', ${index})" class="text-red-400"><i class="fas fa-times"></i></button>
        </div>
    `).join('');
...
function renderPreview() {
    ...
    const expHTML = cvData.experiences.map(exp => `
        <div class="mb-4">
    ...
    const eduHTML = cvData.educations.map(edu => `
    ...
    const langHTML = cvData.languages.map(l => `<li class="flex justify-between text-sm"><span>${l.lang}</span><span class="font-bold">${l.level}</span></li>`).join('');

    if (currentLayout === 'layout-1') {
        container.innerHTML = `<div class="cv-sidebar space-y-6"><div class="mb-4">${imgHTML}</div><div><h3 class="section-title">${t.contact}</h3><div class="text-sm text-start">${cvData.email}<br>${cvData.phone}<br>${cvData.location}</div></div>${skillsHTML ? `<div><h3 class="section-title">${t.skills}</h3>${skillsHTML}</div>` : ''}<div><h3 class="section-title">${t.languages}</h3><ul class="text-sm space-y-1">${langHTML}</ul></div><div><h3 class="section-title">${t.education}</h3>${eduHTML}</div></div><div class="cv-main"><div class="mb-6"><h1 class="text-3xl font-bold theme-text uppercase">${cvData.name}</h1><p class="text-lg text-gray-600">${cvData.title}</p></div><div class="mb-6"><h3 class="section-title">${t.profile}</h3><p class="text-sm text-gray-600 text-start">${cvData.summary}</p></div><div class="mb-6"><h3 class="section-title">${t.experience}</h3>${expHTML}</div></div>`;
    }
    else if (currentLayout === 'layout-2') {
        container.innerHTML = `<div class="cv-sidebar"><div class="mb-6">${imgHTML}</div><div class="mb-8"><h1 class="text-2xl font-bold">${cvData.name}</h1><p class="text-sm opacity-75">${cvData.title}</p></div><div class="mb-8"><h3 class="section-title">${t.contact}</h3><div class="text-sm">${cvData.email}<br>${cvData.phone}</div></div><div><h3 class="section-title">${t.education}</h3>${eduHTML}</div></div><div class="cv-main"><div class="mb-8"><h3 class="section-title">${t.profile}</h3><p class="text-gray-600 text-start">${cvData.summary}</p></div><div class="mb-8"><h3 class="section-title">${t.experience}</h3>${expHTML}</div></div>`;
    }
    else if (currentLayout === 'layout-3') {
        container.innerHTML = `<div class="cv-header-block">${imgSquareHTML}<div><h1>${cvData.name}</h1><p class="opacity-90">${cvData.title}</p></div><div class="ms-auto text-end text-sm">${cvData.email}<br>${cvData.phone}</div></div><div class="cv-body"><div class="full-width"><h3 class="section-title">${t.profile}</h3><p class="text-gray-600 text-start mb-6">${cvData.summary}</p></div><div><h3 class="section-title">${t.experience}</h3>${expHTML}</div><div><h3 class="section-title">${t.education}</h3>${eduHTML}</div></div>`;
    }
    else if (currentLayout === 'layout-4') {
        container.innerHTML = `<div class="cv-header"><div class="cv-header-left">${imgSmallHTML}<div><h1>${cvData.name}</h1><p>${cvData.title}</p></div></div><div class="text-end text-sm text-gray-500">${cvData.email}<br>${cvData.phone}</div></div><div class="mb-8"><h3 class="section-title">${t.profile}</h3><p class="text-gray-600 text-start">${cvData.summary}</p></div><div class="mb-8"><h3 class="section-title">${t.experience}</h3>${expHTML}</div><div><h3 class="section-title">${t.education}</h3>${eduHTML}</div>`;
    }
    else if (currentLayout === 'layout-5') {
        container.innerHTML = `<div class="cv-header">${imgHTML}<h1 class="mt-4">${cvData.name}</h1><p class="text-gray-600">${cvData.title}</p><div class="text-sm text-gray-500 mt-2">${cvData.email} | ${cvData.phone}</div></div><div class="content-grid"><div><h3 class="section-title">${t.profile}</h3><p class="text-sm text-start">${cvData.summary}</p><h3 class="section-title">${t.education}</h3>${eduHTML}</div><div><h3 class="section-title">${t.experience}</h3>${expHTML}</div></div>`;
    }
    else if (currentLayout === 'layout-6') {
        container.innerHTML = `<div class="cv-header">${imgSmallHTML}<div><h1>${cvData.name}</h1><p>${cvData.title}</p></div></div><div class="mb-6"><h3 class="section-title">${t.profile}</h3><p class="text-gray-600 text-start">${cvData.summary}</p></div><div class="mb-6"><h3 class="section-title">${t.experience}</h3>${expHTML}</div><div class="grid grid-cols-2 gap-6"><div><h3 class="section-title">${t.education}</h3>${eduHTML}</div><div><h3 class="section-title">${t.skills}</h3>${skillsHTML}</div></div>`;
    }
    else if (currentLayout === 'layout-7') {
        container.innerHTML = `<div class="cv-header">${imgHTML}<h1 class="mt-4">${cvData.name}</h1><p>${cvData.title}</p><div class="text-sm text-gray-500 mt-2">${cvData.email} | ${cvData.phone} | ${cvData.location}</div></div><div class="content-wrapper"><h3 class="section-title">${t.profile}</h3><p class="text-gray-600 text-start mb-6">${cvData.summary}</p><h3 class="section-title">${t.experience}</h3>${expHTML}<h3 class="section-title">${t.education}</h3>${eduHTML}</div>`;
    }
    else if (currentLayout === 'layout-8') {
        container.innerHTML = `<div class="cv-header"><div><h1>${cvData.name}</h1><p class="opacity-90">${cvData.title}</p></div><div class="text-end text-sm opacity-90">${cvData.email}<br>${cvData.phone}</div></div><div class="cv-body"><div class="cv-sidebar"><h3 class="section-title">${t.contact}</h3><div class="text-sm text-gray-600">${cvData.location}</div><h3 class="section-title">${t.skills}</h3>${skillsHTML}<h3 class="section-title">${t.languages}</h3><ul class="text-sm text-gray-600 space-y-1">${langHTML}</ul></div><div class="cv-main"><h3 class="section-title">${t.profile}</h3><p class="text-gray-600 text-start">${cvData.summary}</p><h3 class="section-title">${t.experience}</h3>${expHTML}<h3 class="section-title">${t.education}</h3>${eduHTML}</div></div>`;
    }
    else if (currentLayout === 'layout-9') {
        container.innerHTML = `<div class="cv-header"><div><h1>${cvData.name}</h1><p>${cvData.title}</p></div><div class="text-end text-sm text-gray-500">${cvData.email}<br>${cvData.phone}<br>${cvData.location}</div></div><div class="two-column"><div><h3 class="section-title">${t.profile}</h3><p class="text-gray-600 text-sm text-start">${cvData.summary}</p><h3 class="section-title">${t.experience}</h3>${expHTML}</div><div><h3 class="section-title">${t.education}</h3>${eduHTML}<h3 class="section-title">${t.skills}</h3>${skillsHTML}</div></div>`;
    }
    else if (currentLayout === 'layout-10') {
        container.innerHTML = `<div class="cv-sidebar"><div class="sidebar-item">${imgHTML}</div><div class="sidebar-item"><h3 class="section-title">${t.contact}</h3><div class="contact-item"><i class="fas fa-envelope"></i> ${cvData.email}</div><div class="contact-item"><i class="fas fa-phone"></i> ${cvData.phone}</div><div class="contact-item"><i class="fas fa-map-marker-alt"></i> ${cvData.location}</div></div><div class="sidebar-item"><h3 class="section-title">${t.skills}</h3>${skillsHTML}</div><div class="sidebar-item"><h3 class="section-title">${t.languages}</h3><ul class="text-sm space-y-1">${langHTML}</ul></div></div><div class="cv-main"><div class="mb-6"><h1 class="text-3xl font-bold" style="color: var(--primary-color)">${cvData.name}</h1><p class="text-lg text-gray-600">${cvData.title}</p></div><div class="mb-6"><h3 class="section-title">${t.profile}</h3><p class="text-gray-600 text-start">${cvData.summary}</p></div><div class="mb-6"><h3 class="section-title">${t.experience}</h3>${expHTML}</div><div><h3 class="section-title">${t.education}</h3>${eduHTML}</div></div>`;
    }
}
...
function saveCV() {
    ...
    link.href = url;
    link.download = cvData.name.replace(/\s+/g, '_') + '_CV.json';
    link.click();
...
function generateCoverLetter() {
    ...
    const letter = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Cover Letter</title>
    <style>body{font-family:Arial,sans-serif;max-width:800px;margin:60px auto;padding:40px;line-height:1.6}
    .name{font-size:24px;font-weight:bold;color:${primaryColor};margin-bottom:20px}
    .footer{text-align:center;margin-top:60px;padding-top:20px;border-top:1px solid #ddd;color:#999;font-size:12px}</style></head>
    <body><div class="name">${cvData.name}</div>
    <p>${cvData.location} | ${cvData.email} | ${cvData.phone}</p>
    <p>${new Date().toLocaleDateString()}</p>
    <p>Dear Hiring Manager,</p>
    <p>I am writing to express my interest in the ${cvData.title} position.</p>
    <p>${cvData.summary}</p>
    <p>Thank you for considering my application.</p>
    <p>Sincerely,<br>${cvData.name}</p>
    <div class="footer">Powered by HashimiHideout</div>
    </body></html>`;
